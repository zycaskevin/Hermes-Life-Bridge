...
class HermesLifeBridge:
    ...
    def _deliver(self, percept, *, hook: str):
        ...
        self.trace.emit(trace_id=trace_id, stage="NORMALIZED", hook=hook,
                        surface=percept.surface, platform=percept.platform,
        ...
        self.trace.emit(trace_id=trace_id, stage="DEDUPE_CHECK", hook=hook,
                        idempotency_key=percept.idempotency_key)
        runtime_event = to_runtime_percept(percept)
        before = time.monotonic()
        try:
            self.trace.emit(trace_id=trace_id, stage="SOCKET_CONNECT", hook=hook,
                            socket=self.config.runtime_socket)
            receipt = self.transport.send_percept(runtime_event)
            elapsed = round((time.monotonic() - before) * 1000, 3)
            self.trace.emit(trace_id=trace_id, stage="EVENT_SENT", hook=hook,
                            latency_ms=elapsed)
            if not receipt.ok:
                self.trace.emit(trace_id=trace_id, stage="FAILED", status="fail",
                                hook=hook, error=receipt.error or "runtime_rejected")
                return receipt
            self.trace.emit(trace_id=trace_id, stage="RUNTIME_ACK", hook=hook,
            ...
            if receipt.persisted and not receipt.duplicate and receipt.state_sequence is not None:
                self.trace.emit(trace_id=trace_id, stage="STATE_ADVANCED", hook=hook,
                                state_sequence=receipt.state_sequence, duplicate=False)
            else:
                self.trace.emit(
                    trace_id=trace_id,
            ...
            self.trace.emit(trace_id=trace_id, stage="FAILED", status="fail",
                            hook=hook, error=type(exc).__name__, detail=str(exc)[:240])
            return None
    ...
    def cli_turn(self, *, session_id: str, turn_id: str, user_message: str):
        ...
        self.trace.emit(trace_id=trace_id, stage="HOOK_RECEIVED",
                        hook="pre_llm_call", platform="cli", session_id=session_id, turn_id=turn_id)
        percept = normalize_cli(
            session_id=session_id, turn_id=turn_id, user_message=user_message, config=self.config
        )
